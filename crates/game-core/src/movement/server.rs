...
/// Apply character movement on the server (authoritative).
/// Uses camera-relative movement matching the client (via replicated CameraYaw).
pub fn apply_server_movement(
    entity: Entity,
    mass: &ComputedMass,
    time: &Time,
    ...
    // === MOVEMENT (Camera-Relative - matches client) ===
    let input = action_state.axis_pair(&CharacterAction::Move).clamp_length_max(1.0);

    // Rotate movement direction by camera yaw (same as client)
